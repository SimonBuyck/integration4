...
test("Create a new GroupStore", () => {
  ...
  const groupsStore = new GroupStore(store);
  expect(groupsStore.groups).toBeInstanceOf(Array);
  expect(groupsStore.groups.length).toBe(0);
});
...
test("Get a group by the id", () => {
  ...
  const id = group.id;
  expect(store.groupStore.resolveGroup(id)).toBe(group);
});
...
test("updateGroupFromServer updates an existing group", () => {
  const store = new RootStore();
  const group = new Group({ name: "testgroup", store: store.groupStore });
  const updatedGroup = store.groupStore.updateGroupFromServer({
    id: group.id,
